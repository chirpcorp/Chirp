...
export const POST = async (request: Request) => {
  // Add better error handling for the entire function
  try {
    ...
    if (eventType === "organization.created") {
      ...
      try {
        ...
        if (!name || !slug || !created_by) {
          console.warn("Missing required organization data:", { name, slug, created_by });
          return NextResponse.json(
    ...
    if (eventType === "organizationInvitation.created") {
      try {
        ...
        console.error("Error handling organization invitation creation:", err);
        return NextResponse.json(
          { message: "Internal Server Error" },
          { status: 500 }
        );
